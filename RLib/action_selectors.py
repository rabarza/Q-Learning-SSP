...
class UCB1ActionSelector(ActionSelector):
    ...
    def select_action(self, agent, state):
        ...
        visits_state_action = np.fromiter((agent.times_actions[state][action] for action in actions), dtype=float)
        ...
        if not_chosen_actions_idx.shape[0] > 0:
            ...
            q_state = np.fromiter((agent.q_table[state][action] for action in actions), dtype=float)
