...
class Grafo:
    ...
    def __init__(self, num_nodos=None, adj_matrix=None):
        ...
        self.__adjacency_matrix = (
            ...
            else self.generar_matriz_adyacencia(self.__density)
        )
    ...
    def generar_matriz_adyacencia(self, density=0.8):
        ...
        k = random.randint(
            1, self.num_nodos - 2
        )  # Número de nodos intermedios entre el nodo inicial y el nodo final (por eso se resta 2)
        nodos_intermedios = random.sample(
            range(1, self.num_nodos - 1), k
        )  # Selección aleatoria de k nodos intermedios

        # Conexiones en secuencia entre los nodos intermedios
        for i in range(k - 1):
            adjacency_matrix[nodos_intermedios[i]][nodos_intermedios[i + 1]] = (
                adjacency_matrix[nodos_intermedios[i + 1]][nodos_intermedios[i]]
            ) = 1

        # Conexiones entre el nodo inicial y el primer nodo intermedio
        adjacency_matrix[0][nodos_intermedios[0]] = adjacency_matrix[
            nodos_intermedios[0]
        ][0] = 1

        # Conexiones entre el último nodo intermedio y el nodo final
        adjacency_matrix[nodos_intermedios[-1]][self.num_nodos - 1] = adjacency_matrix[
            self.num_nodos - 1
        ][nodos_intermedios[-1]] = 1
    ...
    def gen_params_matrix(self, mean=0, std=5, gamma_shape=2, gamma_scale=3):
        ...
        params_matrix = [
            [
                (np.random.normal(mean, std), np.random.gamma(gamma_shape, gamma_scale))
                for i in range(self.num_nodos)
            ]
    ...
    def obtener_costo_arco(self, origen: int, destino: int):
        ...
        if (
            self.adjacency_matrix[origen][destino] == 1
        ):  # Si hay un arco entre los nodos origen y destino
    ...
    def render(self, labeled=False):
        ...
        G = (
            nx.Graph()
        )  # Crear grafo dirigido (DiGraph), grafo no dirigido (Graph) o multigrafo dirigido (MultiDiGraph)
        ...
        for i in range(self.num_nodos):
            for j in range(self.num_nodos):
                ...
                ):  # Si hay un arco entre los nodos i y j
                    distribution = (
                        f"N({self.__params_matrix[i][j][0]:.2f}, {self.__params_matrix[i][j][1]:.2f})"
                        if labeled
                        else ""
                    )
...
class EnvShortestPath:
    ...
    def take_action(self, state, action):
        ...
        links = np.where(self.grafo.adjacency_matrix[state] == 1)[
            0
        ]  # [0] (np.where devuelve un array con los indices de los elementos que cumplen la condicion) pero en una tupla y con [0] se obtiene el primer elemento de la tupla que es el array con los indices
