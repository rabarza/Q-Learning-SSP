...
class CityGraphPlotter:
    def __init__(self, parameters: dict, query: str = "Piedmont, California"):

        self.parameters = parameters
        self.query = query
        self.graph = download_graph(parameters=self.parameters, query=self.query)
        self.gdf_nodes, self.gdf_edges = ox.graph_to_gdfs(self.graph)
        self.gdf_nodes["osmid"] = self.gdf_nodes.index
        self.gdf_nodes["neighbors"] = self.gdf_nodes.apply(
            lambda row: len(list(self.graph.neighbors(row.name))), axis=1
        )
        self.gdf_edges["coordinates"] = self.gdf_edges.apply(
            extract_edge_coordinates, axis=1
        )
